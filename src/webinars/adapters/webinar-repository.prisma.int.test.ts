...
describe('PrismaWebinarRepository', () => {
  ...
  describe('Scenario : repository.findById', () => {
    it('should return a webinar', async () => {
      ...
      // ACT
      const webinarInDatabase = await repository.findById('webinar-id');

      // ASSERT
      expect(webinar).toEqual(webinarInDatabase);
    });

    it('should return null when webinar does not exist', async () => {
      // ACT
      const webinar = await repository.findById('webinar-id-2');
  ...
  describe('Scenario : repository.update', () => {
    it('should update an webinar', async () => {
      // ARRANGE
      const webinar = new Webinar({
        id: 'webinar-id',
        organizerId: 'organizer-id',
      ...
      const updateWebinar = new Webinar({
        id: 'webinar-id',
        organizerId: 'organizer-id',
        title: 'Updated title',
        startDate: new Date('2022-01-01T00:00:00Z'),
      ...
      // ACT
      await repository.update(updateWebinar);

      // ASSERT
      const webinarInDatabase = await prismaClient.webinar.findUnique({
        where: { id: 'webinar-id' },
      });

      expect(webinarInDatabase).toEqual({
        id: 'webinar-id',
        organizerId: 'organizer-id',
        title: 'Updated title',
        startDate: new Date('2022-01-01T00:00:00Z'),
